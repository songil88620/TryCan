...
const CheckoutForm = (props) => {
    ...
    const result = (res) => {
        ...
        if (resultCode == "Ok") {
            ...
            history.push(`/webvieworderstatus/${id}_${items[0].storeId}_${newOrderKey}`);
        } else {
    ...
    const timeHandler = (storeId) => {
        database.ref('user/' + storeId + '/dispensary')
            .on("value", (snapshot) => {
                console.log("snapshot.val()======>", snapshot.val());
                console.log("snapshot.val().email======>", snapshot.val().email);
                var startTime = snapshot.val().storeHours[day].startTime;
                var endTime = snapshot.val().storeHours[day].endTime;
                var openStatus = snapshot.val().storeHours[day].openStatus;
                var online = snapshot.val().storeHours[day].online

                console.log("startTime--->", startTime);

                var start_Time = startTime.split(" ")[0]
                var flag_start_Time = startTime.split(" ")[1]
                var end_Time = endTime.split(" ")[0]
                var flag_end_Time = endTime.split(" ")[1]
    ...
    return (
        <div className="checkout-form-cont">
            <div className="checkout-form">
                <div className="webView-checkout-form-title">
                    <p>Order Summary</p>
                </div>
                <div className="checkout-form-detailed">
                    ...
                    {items
                        .map((item) => <div key={item.id} className="webView-checkout-form-details">
                ...
                <p className="webView-checkout-form-text margin-top-2">Payment</p>
                ...
                <p className="webView-checkout-form-text margin-top-2">Confirm Delivery Address</p>
                <div className="webView-checkout-form-pr">
                    <div className="webView-checkout-form-promo">
                ...
                <p className="webView-checkout-form-text margin-top-2">Special Request</p>
